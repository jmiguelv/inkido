    ...
    async function handleEnrich() {
        ...
        try {
            ...
            const { results } = res.data as {
                results: { pinyin: string; translation: string }[];
            };
            if (results?.[0]) {
                editData.pinyin = results[0].pinyin;
                editData.translation = results[0].translation;
            }
        } catch (e) {
        ...
        <hgroup class="word-hero">
            ...
            {:else}
                ...
                {#if word.character_note}
                    <p class="word-note">{word.character_note}</p>
                {/if}
            {/if}
        </hgroup>
