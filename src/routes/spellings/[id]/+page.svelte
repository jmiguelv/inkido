<script lang="ts">
  ...
  async function handleReorderWord(id: string, direction: 'up' | 'down') {
          ...
          <article class="word-card {getStrokeClass(word.character)}">
            ...
            <div class="card-footer">
              ...
              <div class="reorder-btns">
                <button
                  class="reorder-btn"
                  onclick={() => handleReorderWord(word.id, 'up')}
                  disabled={busy || i === 0}
                  aria-label="Move {word.character} up"
                >↑</button>
                <button
                  class="reorder-btn"
                  onclick={() => handleReorderWord(word.id, 'down')}
                  disabled={busy || i === words.length - 1}
                  aria-label="Move {word.character} down"
                >↓</button>
              </div>
            </div>
